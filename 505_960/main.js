...
async function doJailBreak() {
  ...
  if (config_target === 0x505) { // 5.05
    ...
    if (result === 0 || result === 91) {
      window.log("\nBERHASIL!", "green");
      getPayload672((new URLSearchParams(location.search).get("payload") === "ps4debug") ? "ps4debug.bin" : "payload.bin");
    ...
    // Inject HEN payload
    jb_step_status = await PayloadLoader((new URLSearchParams(location.search).get("payload") === "ps4debug") ? "ps4debug.bin" : "payload.bin"); // Read payload from .bin file
